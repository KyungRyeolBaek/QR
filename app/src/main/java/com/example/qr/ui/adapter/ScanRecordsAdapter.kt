    ...
    class ViewHolder(private val binding: ItemScanRecordBinding) :
        ...
        fun bind(item: ScanRecordDao.ScanRecordWithParticipant) {
            binding.apply {
                ...
                val scanTypeText = if (item.scanType == com.example.qr.data.entity.ScanType.ENTRY) "입장" else "퇴장"
                tvScanType.text = scanTypeText

                val scanTypeColor = if (item.scanType == com.example.qr.data.entity.ScanType.ENTRY) {
