...
import com.example.qr.databinding.ActivityEventBinding
...
class EventActivity : AppCompatActivity() {
    ...
    private fun setupBarcodeInput() {
        // USB QR 코드 리더기는 UsbBarcodeInputHandler로만 처리
    ...
    private fun handleBarcodeScanned(barcodeData: String) {
        try {
            println("🔍 [USB_BARCODE] QR 코드 스캔 시작: $barcodeData")
            ...
            // 상태를 "처리 중"으로 임시 표시
            binding.tvStatus.text = "QR 코드 처리 중..."
            binding.tvStatus.setTextColor(getColor(R.color.status_waiting))

            // 현재 시간 표시로 스캔 확인
            val currentTime = SimpleDateFormat("HH:mm:ss", Locale.getDefault()).format(Date())
            Toast.makeText(this, "USB QR 코드 스캔: $barcodeData ($currentTime)", Toast.LENGTH_SHORT).show()
            ...
            println("❌ [USB_BARCODE] QR 코드 처리 전체 실패: ${e.message}")
            e.printStackTrace()
        }
    }
