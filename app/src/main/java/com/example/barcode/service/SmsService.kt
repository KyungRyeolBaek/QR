...
문의: 02-123-4567"""
        ...
        const val RESEND_MESSAGE_TEMPLATE = """{이름}님의 바코드를 재전송합니다.
첨부된 바코드 이미지를 입장 시 제시해주세요.
IFAA 2025 학회
문의: 02-123-4567"""
        ...
        return try {
            ...
            val intent = Intent(Intent.ACTION_SEND).apply {
                ...
                `package` = getDefaultSmsApp() ?: run {
                    ...
                    Intent.createChooser(this, "바코드 이미지 전송")
                    return@apply
                }
            }
    ...
    fun shareBarcodeFallback(phoneNumber: String, message: String, barcodeImageFile: File): Boolean {
        return try {
            ...
            val shareText = """
                ...
                ※ 이 바코드 이미지를 $phoneNumber 로 전송해주세요.
            """.trimIndent()
            ...
            val chooserIntent = Intent.createChooser(shareIntent, "바코드 이미지 공유")
    ...
    fun getPermissionStatusMessage(): String {
        ...
        return when {
            smsPermission && phoneStatePermission ->
                "✅ 모든 SMS 권한이 허용되었습니다. 바코드 발송이 가능합니다."
            smsPermission && !phoneStatePermission ->
                "⚠️ SMS 발송 권한은 있지만 전화 상태 권한이 없습니다. 일부 기능이 제한될 수 있습니다."
            !smsPermission && phoneStatePermission ->
                "❌ SMS 발송 권한이 없습니다. 바코드를 발송할 수 없습니다."
            else ->
                "❌ SMS 관련 권한이 모두 없습니다. 바코드 발송 기능을 사용할 수 없습니다."
        }
    }
    ...
    fun getPermissionGuideMessage(): String {
        return """
            바코드 SMS 발송을 위해 다음 권한이 필요합니다:

            📱 SMS 발송 권한
            • 참가자에게 바코드 이미지를 MMS로 전송하기 위해 필요
    ...
    /**
     * 바코드 이미지와 함께 MMS 메시지를 발송합니다. (선택적 기능)
    ...
    fun sendBarcodeImageMessage(
        ...
        return try {
            println("🚀 [MMS] 바코드 이미지 메시지 발송 시작")
            println("   📞 수신자: $phoneNumber")
            println("   📊 바코드: $barcodeText")
            ...
            if (!BarcodeImageGenerator.validateBarcodeText(barcodeText)) {
                val errorMsg = "바코드 텍스트가 유효하지 않습니다. ASCII 문자 80자 이하만 지원됩니다."
                println("❌ [검증] $errorMsg (입력: '$barcodeText')")
                updateSendingStatus(phoneNumber, SENDING_STATUS_FAILED, errorMsg)
                return false
            }

            println("✅ [검증] 바코드 텍스트 유효성 검사 통과")
            ...
            // 바코드 이미지 생성
            println("🎨 [이미지] 바코드 이미지 생성 시작...")
            updateSendingStatus(phoneNumber, SENDING_STATUS_SENDING, "바코드 이미지 생성 중...")
     ...
     * @param barcodeText 바코드 텍스트 (fallback용)
    ...
    private fun cleanupBarcodeFile(file: File) {
        try {
            ...
            Thread {
                ...
                if (file.exists()) {
                    file.delete()
                    println("🗑️ 바코드 파일 정리 완료: ${file.name}")
                }
            }.start()
        } catch (e: Exception) {
    ...
    /**
     * 여러 참가자에게 배치로 바코드 메시지를 발송합니다.
     * @param participants 참가자 정보 목록 (이름, 전화번호, 바코드)
    ...
    fun sendBatchBarcodeMessages(
        ...
        println("🚀 배치 바코드 발송 시작: 총 ${participants.size}명")
        ...
        participants.forEachIndexed { index, participant ->
            try {
                ...
                println("📧 배치 MMS 바코드 전송 시작: ${participant.name} (${index + 1}/${participants.size})")

                // 배치 모드로 MMS 텍스트 메시지 + 바코드 이미지 한 번에 전송
                ...
                if (success) {
                    println("✅ 배치 MMS 바코드 전송 성공: ${participant.name}")
                    successCount++
                } else {
                    println("❌ 배치 MMS 바코드 전송 실패: ${participant.name}")
                    failureCount++
                }
