...
class BarcodeImageGenerator {
    ...
    companion object {
        private const val DEFAULT_WIDTH = 600  // 더 큰 크기로 변경
        private const val DEFAULT_HEIGHT = 300  // 더 큰 크기로 변경
        private const val QUIET_ZONE = 20  // 바코드 주변 여백

        /**
         * 바코드 텍스트를 이미지 파일로 생성합니다.
         * @param barcodeText 바코드에 포함될 텍스트
        ...
        fun generateBarcodeImage(
            ...
            return try {
                println("바코드 이미지 생성 시작: $barcodeText")

                // ZXing을 사용하여 바코드 매트릭스 생성
                val writer = MultiFormatWriter()
                val hints = mapOf(
                    com.google.zxing.EncodeHintType.MARGIN to 1  // 최소 여백 설정
                )
                val bitMatrix = writer.encode(barcodeText, BarcodeFormat.CODE_128, width - 2 * QUIET_ZONE, height - 2 * QUIET_ZONE, hints)
                ...
                println("바코드 이미지 생성 완료: ${outputFile.absolutePath}")
                true
            } catch (e: WriterException) {
                println("바코드 생성 오류: ${e.message}")
                false
            } catch (e: IOException) {
        ...
        fun generateBarcodeBitmap(
            ...
            return try {
                println("바코드 Bitmap 생성 시작: $barcodeText")
                ...
                println("바코드 Bitmap 생성 완료")
                bitmap
            } catch (e: Exception) {
                println("바코드 Bitmap 생성 오류: ${e.message}")
                null
            }
        }

        /**
         * 고해상도 스캔 최적화 바코드 이미지를 생성합니다.
         * @param barcodeText 바코드에 포함될 텍스트
         * @return 생성된 파일 객체 또는 null (실패 시)
         */
        ...
        fun generateSmsOptimizedBarcodeToAppFiles(barcodeText: String, context: Context): File? {
            return try {
                println("SMS 최적화 바코드 생성 시작: $barcodeText")

                // MMS 크기 제한을 고려한 최적화된 크기 (통신사별 300KB-1MB 제한)
                val smsWidth = 500  // 조금 더 작게 조정
                val smsHeight = 250
                ...
                if (generateBarcodeImage(barcodeText, barcodeFile, smsWidth, smsHeight)) {
                    ...
                    println("SMS 최적화 바코드 생성 완료: ${barcodeFile.absolutePath} (크기: $fileSize bytes)")
                ...
                println("SMS 최적화 바코드 생성 오류: ${e.message}")
                null
            }
        }

        /**
         * 압축된 MMS 바코드 이미지를 생성합니다.
         */
            ...
            return try {
                ...
                if (generateBarcodeImage(barcodeText, barcodeFile, compressedWidth, compressedHeight)) {
                    ...
                    if (bitmap != null) {
                        ...
                        println("압축된 바코드 생성 완료: $finalSize bytes")
                        barcodeFile
                    } else {
                        barcodeFile.delete()
                        null
                ...
                println("압축 바코드 생성 오류: ${e.message}")
                null
            }
        }
        ...
        /**
         * 바코드 텍스트가 유효한지 확인합니다.
         * @param barcodeText 확인할 바코드 텍스트
         * @return 유효성 검사 결과
         */
        fun validateBarcodeText(barcodeText: String): Boolean {
            return try {
                // CODE_128 형식에 유효한 문자인지 확인
                barcodeText.isNotBlank() &&
                barcodeText.length <= 80 &&  // 일반적인 CODE_128 길이 제한
                barcodeText.all { it.code <= 127 }  // ASCII 문자만 허용
            } catch (e: Exception) {
                false
            }
        }
        ...
        /**
         * 바코드 이미지 생성 테스트 메서드 (디버깅용)
         * @param testText 테스트할 바코드 텍스트
         */
        fun testBarcodeGeneration(testText: String): Boolean {
            return try {
                println("🧪 바코드 생성 테스트 시작: '$testText'")

                // 유효성 검사
                if (!validateBarcodeText(testText)) {
                    println("❌ 바코드 텍스트 유효성 검사 실패")
                    return false
                }
                ...
                if (testFile != null) {
                    ...
                    println("✅ 바코드 생성 성공: ${testFile.absolutePath}")
                    println("📏 파일 크기: $fileSize bytes")
                    println("🔍 스캔 가능한 CODE_128 바코드 이미지 생성됨")

                    // 임시 파일 정리
                    testFile.delete()
                    return true
                } else {
                    println("❌ 바코드 이미지 파일 생성 실패")
                    return false
                }
            } catch (e: Exception) {
                println("❌ 바코드 생성 테스트 오류: ${e.message}")
                false
            }
        }
    }
