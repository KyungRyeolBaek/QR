...
import com.example.barcode.service.SmsService
import com.example.barcode.utils.BarcodeImageGenerator
...
첨부된 바코드 이미지를 입장 시 제시해주세요.
일시: 2024-09-27
문의: 02-123-4567"""

    private var resendTemplate = """{이름}님의 바코드를 재전송합니다.
첨부된 바코드 이미지를 입장 시 제시해주세요.
IFAA 2024 학회
문의: 02-123-4567"""
            ...
            <body>
                ...
                <script>
                        ...
                        const participantsHtml = participants.map(participant => {
                            ...
                            return '<div class="participant-card status-' + statusClass + '">' +
                                ...
                                '<div class="participant-actions">' +
                                    ...
                                    '<button class="action-btn send-btn" onclick="sendBarcode(' + participant.id + ')">바코드발송</button>' +
                    ...
                    function renderParticipantDetailModal(participant) {
                        ...
                        return '<h2 style="margin-bottom: 20px; color: #667eea;">👤 ' + participant.fullName + '</h2>' +
                            '<div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 30px;">' +
                                ...
                                '<div><strong>🎫 바코드:</strong><br><small>' + participant.barcodeData + '</small></div>' +
                            '</div>' +
                    ...
                    function resendBarcode(participantId) {
                        if (!confirm('이 참가자에게 바코드를 재전송하시겠습니까?')) {
                            return;
                        }

                        alert('바코드 재전송 기능은 현재 개발 중입니다.');
                        // TODO: 바코드 재전송 API 구현 후 연결
                    }
                    ...
                    function resendBarcodeToSelected() {
                        const checkedBoxes = document.querySelectorAll('.participant-checkbox-input:checked');
                        if (checkedBoxes.length === 0) {
                            alert('바코드를 재전송할 참가자를 선택해주세요.');
                            return;
                        }

                        if (!confirm(checkedBoxes.length + '명의 참가자에게 바코드를 재전송하시겠습니까?')) {
                            return;
                        }

                        alert('대량 바코드 재전송 기능은 현재 개발 중입니다.');
                        // TODO: 대량 바코드 재전송 API 구현 후 연결
                    }
                    ...
                    async function sendBarcodeWithTemplate(participantId, templateType) {
                        ...
                        try {
                            ...
                            if (response.ok && result.success) {
                                ...
                                alert('✅ ' + participant.fullName + '님에게 바코드를 성공적으로 발송했습니다.');
                            } else {
                                updateParticipantSendStatus(participantId, 'failed');
                                alert('❌ 바코드 발송 실패: ' + (result.error || '알 수 없는 오류'));
                            }
                        } catch (error) {
                            updateParticipantSendStatus(participantId, 'failed');
                            alert('❌ 바코드 발송 중 오류가 발생했습니다.');
                    ...
                    async function sendBarcodeToSelected() {
                        ...
                        if (confirm('선택된 ' + participantIds.length + '명에게 바코드를 발송하시겠습니까?')) {
                            ...
                            alert(participantIds.length + '명에게 바코드 발송이 완료되었습니다.');
                        }
                    }
        ...
        return try {
            ...
            val participantsWithStatus = allParticipants.map { participant ->
                ...
                JSONObject().apply {
                    ...
                    put("entryTime", scanRecords.find { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }?.scanTime ?: 0)
                }
            }
    ...
    private fun serveParticipantDetail(session: IHTTPSession): Response {
        return try {
            ...
            val result = JSONObject().apply {
                ...
                put("entryCount", scanRecords.count { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY })
                put("exitCount", scanRecords.count { it.scanType == com.example.barcode.data.entity.ScanType.EXIT })
            }
    ...
    private fun calculateParticipantStatus(scanRecords: List<com.example.barcode.data.entity.ScanRecord>): String {
        if (scanRecords.isEmpty()) return "NEVER_VISITED"

        val entryRecords = scanRecords.filter { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }
        val exitRecords = scanRecords.filter { it.scanType == com.example.barcode.data.entity.ScanType.EXIT }
    ...
    private fun calculateStayTime(scanRecords: List<com.example.barcode.data.entity.ScanRecord>): Long {
        val entryRecords = scanRecords.filter { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }
        ...
        for (record in allRecords) {
            when (record.scanType) {
                com.example.barcode.data.entity.ScanType.ENTRY -> {
                    entryTime = record.scanTime
                }
                com.example.barcode.data.entity.ScanType.EXIT -> {
                    entryTime?.let { entry ->
    ...
    private fun handleExcelUpload(session: IHTTPSession): Response {
        return try {
            ...
            println("📱 바코드 발송 설정 - 자동발송: $autoSendBarcode, 템플릿: $messageTemplate")
            ...
            try {
                ...
                val participants = mutableListOf<com.example.barcode.data.entity.Participant>()
                ...
                if (activeEvent == null) {
                    val eventId = runBlocking {
                        eventDao.insertEvent(
                            com.example.barcode.data.entity.Event(
                                eventName = "IFAA 2024",
                ...
                for (rowIndex in 1..sheet.lastRowNum) {
                    ...
                    try {
                        ...
                        if (fullName.isNotEmpty() && phoneNumber.isNotEmpty()) {
                            ...
                            participants.add(
                                com.example.barcode.data.entity.Participant(
                                    fullName = fullName,
                ...
                if (autoSendBarcode && participants.isNotEmpty()) {
                    println("🚀 자동 바코드 발송 시작 - ${participants.size}명")
                    ...
                    try {
                        ...
                        val detailMessage = when {
                            sendFailureCount == 0 ->
                                "참가자 ${insertedIds.size}명 추가 및 바코드 발송 완료 ✅ (성공률 100%)"
                            sendSuccessCount == 0 ->
                                "참가자 ${insertedIds.size}명 추가 완료. 바코드 발송 실패 ❌ (모든 발송 실패)"
                            else ->
                                "참가자 ${insertedIds.size}명 추가 완료. 바코드 발송 결과: 성공 ${sendSuccessCount}명, 실패 ${sendFailureCount}명 (성공률 ${successRate}%)"
                        }
                        ...
                        println("❌ 자동 바코드 발송 오류: ${e.message}")
                        e.printStackTrace()

                        val result = JSONObject().apply {
                            put("success", true)
                            put("message", "참가자 ${insertedIds.size}명 추가 완료. 바코드 발송 중 오류 발생: ${e.message}")
                            put("count", insertedIds.size)
    ...
    private suspend fun handleExcelDownload(session: IHTTPSession): Response {
        ...
        return try {
            ...
            val headers = arrayOf("이름", "전화번호", "라이센스", "바코드", "상태")
    ...
    private fun createParticipantDetailSheet(
        ...
        participants: List<com.example.barcode.data.entity.Participant>,
        activeEvent: com.example.barcode.data.entity.Event,
        ...
        participants.forEachIndexed { index, participant ->
            try {
                ...
                row.createCell(7).setCellValue(scanRecords.count { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }.toDouble())
                row.createCell(8).setCellValue(scanRecords.count { it.scanType == com.example.barcode.data.entity.ScanType.EXIT }.toDouble())

                // 최초 입장 시간
                val firstEntry = scanRecords.filter { it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }
        ...
        sheet.setColumnWidth(2, 4000)  // 라이센스번호
        sheet.setColumnWidth(3, 4000)  // 바코드
    ...
    private fun createEnhancedScanRecordSheet(
        ...
        participants.forEachIndexed { participantIndex, participant ->
            try {
                ...
                // 안전한 데이터 조회
                val scanRecords: List<com.example.barcode.data.entity.ScanRecord> = try {
                    runBlocking {
                ...
                val visitSessions = try {
                    ...
                    scanRecords.sortedBy { it.scanTime }.forEach { record ->
                        ...
                        row.createCell(3).setCellValue(when(record.scanType) {
                            com.example.barcode.data.entity.ScanType.ENTRY -> "입장"
                            com.example.barcode.data.entity.ScanType.EXIT -> "퇴장"
                        })
                ...
                visitSessions.forEachIndexed { sessionIndex, session ->
                    ...
                    session.entryTime?.let { entryTime ->
                        val entryRecord = scanRecords.find { it.scanTime == entryTime && it.scanType == com.example.barcode.data.entity.ScanType.ENTRY }
                        if (entryRecord != null) {
                            val row = sheet.createRow(rowIndex++)
                            row.createCell(0).setCellValue(participant.fullName)
                    ...
                    session.exitTime?.let { exitTime ->
                        val exitRecord = scanRecords.find { it.scanTime == exitTime && it.scanType == com.example.barcode.data.entity.ScanType.EXIT }
                        if (exitRecord != null) {
                            val row = sheet.createRow(rowIndex++)
                            row.createCell(0).setCellValue(participant.fullName)
     ...
     */
    private fun analyzeVisitSessions(scanRecords: List<com.example.barcode.data.entity.ScanRecord>): List<VisitSession> {
    ...
    private fun handleSendBarcode(session: IHTTPSession): Response {
        return try {
            ...
            println("📱 개별 바코드 발송 요청 - 참가자 ID: $participantId, 템플릿: $templateType")
            ...
            // 바코드 텍스트를 포함한 최종 메시지 생성
            val finalMessage = """$formattedMessage

🔍 바코드 번호: ${participant.barcodeData}
(입장 시 이 번호를 제시해주세요)"""

            println("📱 최종 전송 메시지: $finalMessage")

            println("📧 MMS 바코드 전송 시작: ${participant.fullName}")

            // MMS로 텍스트 메시지 + 바코드 이미지 한 번에 전송
            ...
            if (sendSuccess) {
                println("✅ MMS 바코드 전송 성공: ${participant.fullName}")
            } else {
                println("❌ MMS 바코드 전송 실패: ${participant.fullName}")
            }

            if (sendSuccess) {
                println("✅ 바코드 발송 성공: ${participant.fullName}")

                val result = JSONObject().apply {
                    put("success", true)
                    put("message", "${participant.fullName}님에게 바코드를 성공적으로 발송했습니다")
                ...
                println("❌ 바코드 발송 실패: ${participant.fullName}")

                val result = JSONObject().apply {
                    put("success", false)
                    put("error", "바코드 발송에 실패했습니다")
                    put("participant_name", participant.fullName)
                    put("participant_id", participantId)
                }
            ...
            println("❌ 바코드 발송 처리 오류: ${e.message}")
            e.printStackTrace()
            newFixedLengthResponse(Response.Status.INTERNAL_ERROR, "application/json",
                JSONObject().put("error", "바코드 발송 처리 실패: ${e.message}").toString())
        }
    }
